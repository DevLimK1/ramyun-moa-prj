...
@WebServlet("/admin/board/review/report")
public class ReportController3 extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		list = service.getReviewReportList(searchName, page);
		req.setAttribute("list", list);
		count = service.getReviewReportCount(searchName);
		req.setAttribute("count", count);

		TilesContainer container = TilesAccess.getContainer(req.getSession().getServletContext());
		container.render("admin.board.review.report", req, resp);
